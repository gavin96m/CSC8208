...
class appDCM:
    ...
    def createProgramScreen(self):
        if self.checkScreenExist("programScreen"):
            ...
            # Battery
            self.batteryLabel = ttk.Label(self.paceSettingFrame, text="Battery status: " + str(Battery(self.programModeCombobox.get()).quantity))
            self.batteryLabel.grid(row=11, column=0, padx=5, sticky=W+ E)
            # self.batterystatus = ttk.Label(self.paceSettingFrame, text = battery.quantity)
            # self.batterystatus.grid(row=11, column=0)
